...
def pos_neg_ray():
    ...
    def error_rate(X,Y,theta_position,S):
        ...
        """
        predict=np.array( [S*np.sign(i-theta_position-0.1) for i in range (1,21)])
        return sum(predict!=Y)/len(Y)
        
    error_pos=[error_rate(X,Y,theta,1) for theta in range(21)]
    error_neg=[error_rate(X,Y,theta,-1) for theta in range(21)]
